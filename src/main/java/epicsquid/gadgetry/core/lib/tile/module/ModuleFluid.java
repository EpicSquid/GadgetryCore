package epicsquid.gadgetry.core.lib.tile.module;
...
public class ModuleFluid extends Module {
  ...
  public class FluidIOProxy extends FluidManager {
    ...
    public FluidStack drain(FluidStack stack, boolean simulate) {
      if (ModuleFluid.this.tile.config.ioConfig.get(face) == FaceIO.OUT || tile.config.ioConfig.get(face) == FaceIO.INOUT
          || ModuleFluid.this.tile.config.ioConfig.get(face) == FaceIO.NEUTRAL) {
    ...
    public int fill(FluidStack stack, boolean simulate) {
      if (ModuleFluid.this.tile.config.ioConfig.get(face) == FaceIO.IN || tile.config.ioConfig.get(face) == FaceIO.INOUT) {
        ModuleFluid.this.tile.markDirty();
              ...
              if (t != null && t.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, f.getOpposite())) {
                ...
                if (s != null) {
                  ...
                  if (amount > 0) {
                    ...
                    tile.markDirty();
                    t.markDirty();
                    ELEvents.markForUpdate(tile.getPos().offset(f), t);
                    break;
                  }
                }
              }

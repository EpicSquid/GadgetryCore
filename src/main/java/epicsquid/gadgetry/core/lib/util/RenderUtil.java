package epicsquid.gadgetry.core.lib.util;
...
public class RenderUtil {
  public static ResourceLocation beam_texture = new ResourceLocation(GadgetryCore.MODID + ":textures/effect/beam.png");
  public static ResourceLocation glow_texture = new ResourceLocation(GadgetryCore.MODID + ":textures/effect/glow.png");
  ...
  public static void renderStarBurst(BufferBuilder buf, double x, double y, double z, float r, float g, float b, float a, double width, double radius) {
    for (double i = -Math.PI * 0.5; i <= Math.PI * 0.5; i += Math.PI * 0.125) {
      for (double j = 0; j < Math.PI * 2.0; j += Math.PI * (0.75 * (Math.abs(i / (Math.PI * 0.5))) + 0.25)) {
        double si = ((float) (ELEvents.ticks) / 80.0f) * (0.375f + 0.773 * j / (Math.PI * 2.0)) * (0.25f + 1.225 * i / (Math.PI)) * Math.PI * 2.0;
        double sj = ((float) (ELEvents.ticks) / 57.0f) * (0.25f + 1.63 * i / (Math.PI * 0.5)) * (0.375f + 0.241 * j / (Math.PI)) * Math.PI * 2.0;

package epicsquid.gadgetry.core.lib.gui;
...
public class ElementVerticalProgressBar implements IGuiElement {
  ...
  public ElementVerticalProgressBar(int x, int y, int u1, int v1, int w, int h, int u2, int v2, int[] data, ResourceLocation texture) {
    this.x = x;
    this.y = y;
    this.u1 = u1;
    this.v1 = v1;
  ...
  public void draw(GuiContainer g, float partialTicks, int mouseX, int mouseY) {
    ...
    if (data.length >= 2) {
      ...
      g.drawTexturedModalRect(g.getGuiLeft() + x, g.getGuiTop() + y + (int) Math.round((float) h * coeff), u2, v2 + (int) Math.round((float) h * coeff), w,
          (int) Math.round((float) h * (1.0f - coeff)));
    }
  }
